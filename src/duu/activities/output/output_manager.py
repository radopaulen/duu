...
class OutputManager:
    ...
    def change_weights(self, weights):
        results_buffer = self.read_results()
        for i, sample in enumerate(results_buffer["samples"]):
            sample["w"] = weights[i]
        with open(self.results_path_filename_extn, 'wb') as file:
            pickle.dump(results_buffer, file)

    def change(self, paths, contents):
        results_buffer = self.read_results()
        for i, path in enumerate(paths):
            results_buffer[path] = contents[i]
        with open(self.results_path_filename_extn, 'wb') as file:
            pickle.dump(results_buffer, file)
        # TODO Improve for nested items that could also be lists etc :(