...
from duu.activities.output import DesignSpaceOutputManager
...
class DesignSpaceManager(ActivityManager):
    ...
    def check_problem(self, problem):
        ...
        mandatory_keys = ['constraints',
                          'parameters_samples', 'parameters_best_estimate',
                          'design_variables', 'target_reliability']
        assert all(mkey in problem.keys() for mkey in mandatory_keys), \
            "The 'problem' keys must be the following:\n" \
            "'constraints', 'parameters_samples', 'parameters_best_estimate'" \
            ", 'design_variables', 'target_reliability']." \
        ...
        for i, p_sample in enumerate(p_samples):
            ...
            mandatory_keys = ['c', 'w']
            assert all(mkey in p_sample.keys() for mkey in mandatory_keys), \
            "A 'parameters_samples' item keys must be the following:\n" \
            "'c' - a list of coordinates, 'w' - a scalar reperesenting a weight." \
            "Look for typos, white spaces or missing keys."
        ...
        assert (len(p_best_estimate) == len(p_samples[0]['c'])), \
